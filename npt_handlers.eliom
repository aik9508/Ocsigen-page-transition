...
let upload_user_avatar_handler myid () ((), (cropping, photo)) =
  let avatar_dir =
    List.fold_left Filename.concat
      (List.hd !Npt_config.avatar_dir)
      (List.tl !Npt_config.avatar_dir) in
  let%lwt avatar =
  ...
  | e ->
    ...
    then
      ...
      then
        ...
        in
        Npt_base.App.send
          (Npt_page.make_page (Os_page.content page))
      else
        let page = [ div
...
let%shared settings_handler myid_o () () =
  let%lwt content = match myid_o with
    | Some _ -> Npt_settings.settings_content ()
    | None -> Lwt.return [ p [%i18n log_in_to_see_page ~capitalize:true]]
  in
  Npt_container.page myid_o content

let%server update_language_handler () language =
  Os_session.connected_wrapper Npt_language.update_language
    (Npt_i18n.language_of_string language)

let%client update_language_handler () language =
  Npt_i18n.(set_language (language_of_string language));
